...
Definition _sim (sim: SIM_CFG) (ρ1_src ρ1_tgt : cfg bor_lang) : Prop :=
  ...
  (terminal ρ1_tgt.1 → ∃ T2_src σ2_src,
      rtc τstep (ρ1_src.1, σ1_src) (T2_src, σ2_src) ∧
      sim_state σ2_src σ1_tgt ∧ terminal T2_src) ∧
  ...
  (∀ ρ2_tgt tid_tgt o, one_step (ρ1_tgt.1, σ1_tgt) tid_tgt o ρ2_tgt →
      ∃ ρ2_src tid_src, one_step (ρ1_src.1, σ1_src) tid_src o ρ2_src ∧
...
Proof.
  ...
  intros ρ2_tgt tid o ONE.
  destruct (STEP _ _ _ ONE) as [ρ2_src [tid_src [ONE_src [SS2 Hr]]]].
  exists ρ2_src, tid_src. do 2 (split; [done|]). by apply LE.
Qed.
...
Proof.
  ...
  destruct (STEP _ _ _ ONE) as (e2_src & σ2_src & ONE_src & SS' & Hr).
