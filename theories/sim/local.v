...
| sim_local_body_step e_src σ_src e_tgt σ_tgt
    (STEP: ∀ r_f e_tgt' cfg_tgt'
      ...
      (VALID: ✓ (r_f ⋅ r)) (WSAT: wsat (r_f ⋅ r) σ_src σ_tgt)
      (* if tgt can locally make 1 step *)
      (STEPT: (e_tgt, (mkConfig fns_tgt σ_tgt)) ~t~> (e_tgt', cfg_tgt')),
...
| sim_local_body_step_over_call K_src K_tgt fid e_src e_tgt el_tgt σ_src σ_tgt
    ...
    (STEPOVER: ∀ r_f
        ...
        (e_src, (mkConfig fns_src σ_src)) ~t~>*
          (fill K_src (Call #[ScFnPtr fid] el_src), cfg_src) ∧
        ...
        (∀ r_f' r' v_src v_tgt cfg_src' cfg_tgt'
          ...
          (VALID': ✓ (r_f' ⋅ (rc ⋅ r')))
          (WSAT' : wsat (r_f ⋅ (rc ⋅ r')) cfg_src'.(cst) cfg_tgt'.(cst))
...
| sim_local_body_end_call v_src σ_src v_tgt σ_tgt e_src e_tgt
    ...
    (END: ∀ r_f cfg_tgt'
        ...
        (WSAT : wsat (r_f ⋅ r) σ_src σ_tgt)
        (* if tgt can end call *)
        (ETGT: (EndCall (Val v_tgt), (mkConfig fns_tgt σ_tgt)) ~t~> cfg_tgt'),
...
Record sim_prog (prog_src prog_tgt: fn_env) := {
  ...
  sim_prog_main:
    sim_local_body wsat vrel prog_src prog_tgt ε
                   (Call #["main"] []) init_state (Call #["main"] []) init_state;
}.
End program.
